...
def main(args):
    ...
    if args.finetune and not args.eval:
        ...
        # MyVit 的架构设计中只有 head 是新增的
        assert set(msg.missing_keys) == {'head.weight', 'head.bias'}, f"Missing keys found: {msg.missing_keys}"
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir:
            misc.save_model(
                args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                loss_scaler=loss_scaler, epoch="latest") # 文件名固定为 checkpoint-latest.pth
        ...
        if test_stats["acc1"] > max_accuracy:
            if args.output_dir:
                misc.save_model(
                    args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                    loss_scaler=loss_scaler, epoch="best") # 文件名固定为 checkpoint-best.pth
